...
def handle_pre_tool_use(
    ...
    *,
    ...
    try:
        ...
        return _decision("deny", f"Quarterdeck gate rejected malformed hook input: {exc}")
    ...
    with gate_lock(ledger):
        ...
        if collision is not None:
            ...
            return _decision("deny", "Quarterdeck request identity mismatch")
        if state is None:
            request_id = new_ulid()
            ...
            if created is None:
                return _decision("deny", "Quarterdeck could not durably record approval request")
            return _decision("defer", f"Quarterdeck approval pending: {request_id}")
        if state.terminal is not None or state.consumed is not None:
            return _decision("deny", "Quarterdeck approval was already consumed or closed")
        if _is_expired(state, now):
            ...
            return _decision("deny", "Quarterdeck approval expired")
        if state.decision == "approved":
            ...
            if consumed is None:
                return _decision("deny", "Quarterdeck could not durably consume approval")
            return _decision("allow", f"Quarterdeck approval consumed: {state.request_id}")
        if state.decision is not None:
            return _decision("deny", f"Quarterdeck approval decision: {state.decision}")
        return _decision("defer", f"Quarterdeck approval pending: {state.request_id}")

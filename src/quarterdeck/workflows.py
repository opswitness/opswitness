...
shell-free argv registered in ``workflows.yaml``. Quarterdeck owns dispatch evidence,
...
from quarterdeck.redact import redact_argv
...
def _worker_environment(settings: Settings) -> dict[str, str]:
    ...
    env["QD_CONFIG_DIR"] = str(config_dir())
    env["QD_LEDGER_DIR"] = str(settings.ledger_dir)
    return env

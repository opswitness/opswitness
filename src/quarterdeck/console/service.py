...
from quarterdeck.console.store import (
    BlueprintNotFound,
    PlanNotFound,
    PlanStore,
    TaskTemplateStore,
...
ALWAYS_SAFE_AION_TOOLS = frozenset({"mcp__quarterdeck__qd_request_input"})
AUTOMATIC_SAFE_AION_TOOLS = frozenset(
    {
        ...
        "mcp__quarterdeck__qd_artifact_verify",
        "mcp__quarterdeck__qd_artifacts",
        "mcp__quarterdeck__qd_fleet_status",
        "mcp__quarterdeck__qd_projection_backlog",
        "mcp__quarterdeck__qd_python_package_status",
        "mcp__quarterdeck__qd_run_events",
        "mcp__quarterdeck__qd_runs",
        "mcp__quarterdeck__qd_watchdog",
        "mcp__quarterdeck__qd_workflow_status",
        "mcp__quarterdeck__qd_workflows",
    }
)
...
class ConsoleService:
        ...
        for provider, probe in probes.items():
            ...
            if provider == "deepseek":
                ...
                privacy = (
                    ...
                    else "厂商 API 登录；Quarterdeck 不保存密钥"
                    if auth_mode == "console"
                    else "厂商官方登录；Quarterdeck 不接收账号密码"
                )
        ...
        *,
        ...
        self._decide_approval_locked(
            ...
            ApprovalDecisionRequest(
                decision="approve",
                decision_note=(
                    f"Quarterdeck automatic tool policy v{AUTO_APPROVAL_POLICY_VERSION}: {reason}."
                ),
                confirmed=True,
            ),
            source=(
            ...
            if not reconciled:
                try:
                    ack = self.aion.send_team_message(
                        ...
                        (
                            f"{marker}\n"
                            "Continue the selected prior Quarterdeck run in the same confirmed "
                            ...
                            "artifact filename; Quarterdeck will hash and bind only this run's delta. "
            ...
            try:
                ...
                if not reconciled:
                    self.aion.send_team_message(
                        ...
                        (
                            f"{marker}\n"
                            "The operator answered the pending Quarterdeck question below. Treat the "
    ...
    def _telegram_environment_controlled() -> bool:
        return any(
            name in os.environ for name in ("QD_TELEGRAM__BOT_TOKEN", "QD_TELEGRAM__CHAT_ID")
        )

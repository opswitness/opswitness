...
def workflow_list() -> None:
    ...
    from opswitness.workflows import workflow_catalog
...
def service_render(
    ...
    from opswitness.service import render_launchd, write_launchd
...
def backup_create(
    ...
    from opswitness.backup import create_backup
...
def backup_restore(
    ...
    from opswitness.backup import restore_backup
...
def console_serve(
    ...
    from opswitness.console.access import console_public_url, validate_private_tls
    from opswitness.console.app import create_app
    from opswitness.console.service import ConsoleService, ConsoleUnavailable
...
def console_pair() -> None:
    ...
    from opswitness.console.access import console_public_url
    from opswitness.console.pairing import DevicePairingStore, PairingStateError
...
def telegram_configure(
    ...
    from opswitness.config import save_telegram_credentials
...
def telegram_test() -> None:
    ...
    if not send_telegram("OpsWitness Telegram delivery test", _load_settings_cli()):
...
def _effective_soak_schedules() -> list[dict]:
    ...
    from opswitness.config import config_dir
...
def soak_start(
    ...
    from opswitness.soak import record_contract
...
def soak_reset(
    ...
    from opswitness.ledger import Ledger
    from opswitness.soak import contract_details, record_contract
...
def soak_status(
    ...
    from opswitness.ledger import Ledger
    from opswitness.soak import evaluate_ledger_soak
...
def soak_checkpoint(name: str) -> None:
    ...
    from opswitness.ledger import Ledger
    from opswitness.soak import record_checkpoint
...
def gate_install() -> None:
    """Write OpsWitness-owned isolated Claude settings (never ~/.claude settings)."""
    from opswitness.gated_claude import install_gate_settings
...
def gate_hook(post: str = typer.Option("", "--post")) -> None:
    ...
    from opswitness.gate import handle_post_tool_use, handle_pre_tool_use
    from opswitness.ledger import Ledger
    ...
    try:
        ...
        if post:
            typer.echo(f"OpsWitness post-hook evidence failure: {exc}", err=True)
            raise typer.Exit(code=1) from None
        response = {
            "hookSpecificOutput": {
                "hookEventName": "PreToolUse",
                "permissionDecision": "deny",
                "permissionDecisionReason": f"OpsWitness gate failed closed: {exc}",
            }
        }
...
def gate_recover(once: bool = typer.Option(True, "--once/--loop")) -> None:
    ...
    from opswitness.gated_claude import recover_once
...
def gated_claude_command(
    ...
    from opswitness.gated_claude import run_gated_claude
...
def artifacts_register(
    ...
    from opswitness.artifacts import register_artifact
    from opswitness.ledger import Ledger
...
def artifacts_list(run_id: str = typer.Option("", "--run-id")) -> None:
    ...
    from opswitness.artifacts import artifact_records
    from opswitness.ledger import Ledger
...
def artifacts_show(event_id: str) -> None:
    ...
    from opswitness.artifacts import registration
    from opswitness.ledger import Ledger
...
def artifacts_verify(event_id: str = typer.Argument("")) -> None:
    ...
    from opswitness.artifacts import artifact_records, registration, verify_registration
    from opswitness.ledger import Ledger
...
def artifacts_eval(
    ...
    from opswitness.artifacts import evaluate_artifact
    from opswitness.ledger import Ledger
...
def artifacts_signoff(
    ...
    from opswitness.artifacts import signoff_artifact
    from opswitness.ledger import Ledger
...
def runs(
    ...
    from opswitness.index import query_runs, rebuild
    from opswitness.ledger import Ledger
...
def status() -> None:
    ...
    from opswitness.index import job_summary, rebuild
    from opswitness.ledger import Ledger
...
def project() -> None:
    ...
    from opswitness.config import resolve_api_key
...
def adopt_launchd(
    ...
    from opswitness.adopt import apply, plan, resolve_qd_bin, rollback
...
def init(
    ...
    from opswitness.bootstrap import init_workspace
    from opswitness.config import config_dir
...
def digest(
    ...
    if html_out:
        from opswitness.digest import render_page_html
    ...
    if telegram:
        from opswitness.digest import render_telegram_html
        from opswitness.notify.telegram import send_telegram

...
ANTHROPIC_KEYCHAIN_SERVICE = "com.opswitness.anthropic-api-key"
LEGACY_ANTHROPIC_KEYCHAIN_SERVICE = "com.quarterdeck.anthropic-api-key"
...
_LEGACY_PROVIDER_KEYCHAIN_SERVICES: dict[ApiProvider, str] = {
...
def _helper_payload(account: str, service: str) -> bytes:
    return (
        "#!/bin/sh\n"
        "set -eu\n"
        f'exec {_SECURITY_BIN} find-generic-password '
        f'-a {account} -s {service} -w\n'
    ).encode()


def _managed_helper_service(
    helper: Path,
    *,
    account: str,
    services: tuple[str, ...],
    ...
    try:
        ...
        if (
            ...
            or metadata.st_size > _MAX_HELPER_BYTES
        ...
        return None
    return next(
        (service for service in services if payload == _helper_payload(account, service)),
        None,
    )


def managed_provider_api_key_helper(settings: Settings, provider: ApiProvider) -> Path | None:
    """Return only a regular, executable helper installed under OpsWitness state."""
    helper = provider_api_key_helper_path(settings, provider)
    service = _managed_helper_service(
        helper,
        account=PROVIDER_KEYCHAIN_ACCOUNT,
        services=(
            _PROVIDER_KEYCHAIN_SERVICES[provider],
            _LEGACY_PROVIDER_KEYCHAIN_SERVICES[provider],
        ),
    )
    if service is None:
        return None
...
def configure_provider_api_key(
    ...
    *,
    ...
    atomic_write(
        helper,
        _helper_payload(PROVIDER_KEYCHAIN_ACCOUNT, service),
        mode=0o700,
    )
    return True
...
def managed_anthropic_api_key_helper(settings: Settings) -> Path | None:
    """Return only a helper installed and still owned by this OpsWitness state directory."""
    ...
    try:
        ...
        if configured != str(helper):
            return None
    except (OSError, ProviderCredentialError):
        return None
    service = _managed_helper_service(
        helper,
        account=ANTHROPIC_KEYCHAIN_ACCOUNT,
        services=(ANTHROPIC_KEYCHAIN_SERVICE, LEGACY_ANTHROPIC_KEYCHAIN_SERVICE),
    )
    if service is None:
        return None
    return helper
...
def configure_anthropic_api_key(
    ...
    *,
    ...
    atomic_write(
        helper,
        _helper_payload(ANTHROPIC_KEYCHAIN_ACCOUNT, ANTHROPIC_KEYCHAIN_SERVICE),
        mode=0o700,
    )

...
def _template_bytes(name: str) -> bytes:
    ...
    return (
        files("opswitness")
        .joinpath("templates", "quant-fleet", "launchd", f"com.opswitness.{name}.plist")
        .read_bytes()
    )
...
def render_launchd(name: str, settings: Settings) -> bytes:
    ...
    rendered = _replace(
        data,
        {
            "__OPSWITNESS_BIN__": str(qd_bin),
            "__OPSWITNESS_CONFIG_DIR__": str(config_dir().resolve()),
            "__OPSWITNESS_LOG_DIR__": str(settings.services.log_dir.expanduser().resolve()),
        },
    )
...
def build_service_exec(
    ...
    if name not in SERVICE_NAMES:
        raise ValueError(f"unknown service {name!r}; expected one of {SERVICE_NAMES}")
    env = dict(os.environ)
    env["OPSWITNESS_CONFIG_DIR"] = str(config_dir().resolve())
    if name == "paperclip":
        ...
        if not settings.database_url:
            raise ValueError("database_url is missing from secrets.yaml or OPSWITNESS_DATABASE_URL")

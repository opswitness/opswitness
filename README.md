...
- **Your credentials stay yours.** Quarterdeck never handles Claude subscription tokens; it talks to
  ...
  Quarterdeck probes only their fixed loopback APIs, requires explicit confirmation before starting
...
AionUi, Paperclip, and provider CLIs remain loopback-only. Quarterdeck can sit behind a trusted
...
SHA-256 in Quarterdeck's ledger, and creates a new child plan, plan hash, Paperclip issue, and run.
...
receives an automatic single-use decision. Quarterdeck still creates the Paperclip approval and
